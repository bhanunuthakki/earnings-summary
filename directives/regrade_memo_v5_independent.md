...
## Post-grade update — v5 fixes landed (2026-06-01, same day)

After this grade was written, the findings above were **fixed and merged** as seven phased, CI-green PRs (#258–#264), each gated by the full pytest suite + the diff-aware ruff gate + the new pyright ratchet. The grade itself stands as written — this records the remediation, and closes the "one operational step from vanishing" gap the closing predicted.

- **Data reliability:** migration **0069** recreates `brief_provenance_log` (dropped in 0031, but the `build_artifacts` writer still targeted it), and the **real prod DB was upgraded `0067 → 0069`** (backed up first). The 0068 substrate CHECK constraints + the `uq_alerts_active_signature` dedup index now **enforce in production** — verified on a copy and on prod (a bad-`status` insert is rejected; row counts preserved; FK clean). The AMZN NULL `threshold_direction` (finding #2) was confirmed **not a bug**: the seeder deliberately left it ungrounded (`is_thesis_breaker=0`; notes: "no consolidated threshold can be grounded"), and 0068's CHECK permits `NULL` by design. *(#263)*
- **Quality:** **pyright is now a blocking CI job** — a diff-aware `typecheck` ratchet (whole-repo strict count vs. base via a base worktree; the ~3088 baseline tolerated, fails only on new errors) *(#260)*. The validation engine is now **enforceable** via `run_validation_engine.py --gate` (exits non-zero on any HALT-severity issue); a per-row insert gate is structurally impossible since the checks are population-level *(#264)*.
- **Smart caching:** the dominant FMP fetch path now logs to `source_calls` (`registry.log_calls_batch` + instrumented `run_ticker`), so FMP cache-hit-rate is measurable from real data *(#258)*. The **SayDo batch cron is now registered** in Task Scheduler, so the 50% discount is live (finding #5).
- **LLM pass-through:** a centralized `prompt_versions` registry replaces the scattered hardcoded `"v1"`, making the calibration A/B machinery live-able from one bump point *(#262)*.
- **Richness & surfacing:** `/digest`, `/feed`, `/alerts` are now **live command-center routes** with topbar links (no longer static-file-only) *(#259)*, plus a Personal-CIO **xlsx export** *(#261)*.

Still genuinely deferred (unchanged, and honestly so): trigger-purpose calibration *grading* (needs sound per-purpose ground truth; overlaps the predictions lane — the registry above unblocks it), and **live firing breadth** — still one alert / one news row, which is a runtime/data matter, not code. The substrate and surfacing this grade docked are now real *behavior*, not just capability; the firing breadth that caps the product score is the honest remaining work.
