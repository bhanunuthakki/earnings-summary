...
|---|---|---|---|
| `run_morning_pipeline` | daily 04:00 | stage 0 news → `material_news_classification`; stage 0b `decision_conditions_extract` (+ qualitative twin); stage 1b standup (`run_standup.py`) → `ask_answer` compose + `eval_judge` gate per surviving trip; stage 1c `pre_earnings_brief` (Sonnet, one call per name reporting within 7d — portfolio + `auto_pre_earnings_brief`-marked evaluation names; llm_artifacts input-hash cache + T-1 refresh gate bound each name to ≤2 calls per earnings cycle, so a normal morning is 0-2 calls; budget skip-mode $5/mo, 0260; per-item degrade — transient defers the ticker, tallied `deferred_transient`, retried next morning; hard stops exit 2; added 2026-07-31, owner ruling relaxing D2 for this one artifact); stage 1d `post_earnings_readout` (Sonnet, active portfolio names ONLY; evaluation names never enter the scheduled query and spend only on explicit owner request; one current artifact per selected transcript `period_end`, input-hash cache makes unchanged mornings zero-call; budget skip-mode $5/mo, 0273; transient per-item defer, hard stops exit 2; added 2026-08-04) | the canonical protected window; post-#814 degrades per-item; stage 1b was the previously-undocumented leg behind the 2026-07-13 incident (below) |
...
| `weekly_p2_lens_refresh` | Friday 22:00 | `lens:five_min_reread`, `lens:thesis_drift_qoq` for P2 only | production plan measured at 91 pairs on 2026-08-09; P3 scheduled LLM work is disabled; plan fails closed above 128; calls dispatch only inside 21:30–01:35 PT so the 20-minute backend ceiling cannot spill into Saturday's 02:00 jobs or the 03:00–05:00 protected window; transient failures defer per item and return retry status 75; hard stops return 2 |
...
| `session_distill` | **manual only; no scheduled task** | Ordinary invocation runs `session_distill` only (Sonnet, one structured call per eligible undistilled session). `exit_postmortem_draft` is a separate, explicit operator action available only through `--postmortem-backfill`; it never piggybacks on session distillation. | Disabled 2026-08-10 after production evidence showed no eligible Ask/raw/postmortem candidates and no useful scheduled output. Keep per-item degrade, hard stops, and the existing purpose budgets. Re-enable scheduling only after bounded dry-runs show a non-zero representative candidate corpus and useful adopted outcomes, the owner explicitly approves the spend, a collision-free window is registered here, and generated Scheduler artifacts are restored. A scheduled job must remain one-purpose: no postmortem second leg. |
