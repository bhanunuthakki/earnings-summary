...
| `compose_senior_partner_brief` | Sunday 09:00 (weekly synthesis) | `senior_partner_brief` (model TBD via the cheapest-at-parity eval loop, PRD §10.1/§10.5 — one governed structured call per week) | **NOT YET MERGED** (P2.2, PRD §9.1 — `execution/compose_senior_partner_brief.py` and `src/advisor/senior_partner_brief.py` are implemented and in review as PR #1002, not yet on main as of 2026-07-25; this row pre-registers the window per the PRD §10.4 rule that new scheduled LLM work registers its cadence here). Slot clear of the 03:00–05:00 window, the Sun 08:00 `weekly_packet` send (which this composes over), and the Sun 10:30 eval rungs. Composes over `weekly_packet`, the latest valid Risk Budget (refreshed daily at 04:00 by stage 0i — not by this job), the latest Incremental Dollar Recommendation, Investment Decision Cards, Worldview, and decision/calibration history (PRD §9.1). Budget: `on_exceed='block'`, not `warn` — a budget-exhausted brief must show explicit unavailability, never a degraded/synthesized brief (PRD §10.6). Immediate/exceptional triggers (likely thesis break, materially changed risk, stale critical data, an owner-marked time-sensitive decision) are event-driven, not this weekly cron row, and must independently follow rule 3 per-item degrade. **Remove the NOT-YET-MERGED flag and register the Scheduled Task once #1002 lands** — the script ships unscheduled by design until then. |
