...
## 4. Coverage gaps / open decisions (the critic flagged these — no session owns them yet)

Each needs an owner call before it gets (or doesn't get) a session:

1. **Ask conversational latency / prompt caching** — the single largest goal-relevant gap with ZERO owning session; transport is a cold `claude -p` that re-encodes the whole thread every turn, no caching — and every Opus bet here ADDS LLM calls on top. Default: **defer to a dedicated follow-on program** (it's a transport rewrite, orthogonal to loop-closing) — but flagged because the Wave-C bets worsen it.
2. **Conviction-as-probability (Brier)** — true calibration ("when I say 70% I'm right 70%") needs a numeric predicted-probability per decision; conviction is a 3-bucket text enum. Default: **answer "am I getting better" with hit-rate trends (cheap, no schema)**; numeric calibration is a schema+UX change deferred unless the owner wants it.
3. **Cross-session advisory memory** — recall "last week you concluded X about NU"; only stubbed by L9's lightweight note. The hazard: naive memory re-asserts a conclusion the current-state packs have since invalidated. Default: **stub in L9, design a real per-ticker conclusion store as a follow-on** if the stub proves insufficient.
4. **Tax-lot / options reasoning in Ask** — tax lots exist in the tracker client but aren't in the holdings pack; no options in the position model. Default: **surface tax lots into the holdings pack (cheap, data exists); options/derivatives deferred** (no position-model support).
5. **Agentic recompute-from-chat** — wire the new L4 `/api/dcf/recompute` as a tool the Ask loop can call ("recompute the DCF with 8% WACC" from a conversation). Default: **a one-seam follow-on after L4**; fold into L9 or its own micro-session.
