...
def test_lane_partition_is_total_and_disjoint() -> None:
    assert NEWS_MIRRORED_TYPES | DIET_ONLY_TYPES | RETIRED_SIGNAL_TYPES == SIGNAL_TYPES
    assert not (NEWS_MIRRORED_TYPES & DIET_ONLY_TYPES)
    assert not ((NEWS_MIRRORED_TYPES | DIET_ONLY_TYPES) & RETIRED_SIGNAL_TYPES)
    ...
    assert {"media_appearance"} == RETIRED_SIGNAL_TYPES
...
def test_predicates() -> None:
    ...
    assert not is_diet_only("media_appearance")
    assert not is_news_mirrored("media_appearance")
