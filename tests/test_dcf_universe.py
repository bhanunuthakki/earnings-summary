...
def _make_repo(tmp_path: Path, rows: list[tuple[str, str]]) -> Path:
...
def test_build_all_default_tickers_unions_eval_and_workbooks(tmp_path: Path) -> None:
    ...
    assert build_all_redesigned_dcf.default_tickers(repo) == ["AMZN", "NU", "UBER"]
...
def test_refresh_all_named_unions_legacy_wacc_holdings(tmp_path: Path) -> None:
    ...
    assert refresh_dcf.dcf_maintained_universe(repo) == ["META", "UBER"]
