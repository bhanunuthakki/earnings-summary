...
def test_0265_writer_reservation_closes_downgrade_empty_check_race(
    tmp_path: Path,
) -> None:
    database = tmp_path / "ontology-operation-ledger-lock.db"
    with sqlite3.connect(database) as conn:
        conn.execute(
            "CREATE TABLE metric_ontology_operation_ledger (operation_id TEXT PRIMARY KEY)"
        )
