...
if "%~1"=="--all-tracked" (
  "%REPO_ROOT%\cron\run_python.bat" "refresh_news" "portfolio-db" "%REPO_ROOT%\execution\refresh_news.py" --all-tracked
) else (
  if "%~2"=="" (
    "%REPO_ROOT%\cron\run_python.bat" "refresh_news" "portfolio-db" "%REPO_ROOT%\execution\refresh_news.py" --ticker %1
  ) else (
    "%REPO_ROOT%\cron\run_python.bat" "refresh_news" "portfolio-db" "%REPO_ROOT%\execution\refresh_news.py" --ticker %1 --news-days %2
  )
)
exit /b %ERRORLEVEL%
