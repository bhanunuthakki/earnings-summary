...
| `earnings-summary\track_comp_metrics` | Daily 06:45 | `track_comp_metrics.task.xml` | `run_track_comp_metrics.bat` | **Comparable-sets Phase 2 daily metrics** (`docs/design/comparable_sets_bottoms_up.md` §8/§11). Wraps `python execution/track_comp_metrics.py --all-tracked`: computes + upserts `scope_type='comparable_set'`/`'industry'`/`'sector'` rows into `comp_set_metrics_daily` for every portfolio+watchlist+evaluation subject with a frozen comparable set, plus every industry/sector present in the pool — market cap moves daily even though quarterly financials don't. Zero LLM. Requires `execution/build_comparable_sets.py --all-tracked` to have been run at least once (a subject with no frozen set is logged and skipped, not an error). |
| `earnings-summary\check_comp_set_drift` | Weekly, Sunday 09:00 | `check_comp_set_drift.task.xml` | `run_check_comp_set_drift.bat` | **Comparable-sets Phase 2 QA drift check** (§7). Compares our bottoms-up industry/sector PE against FMP's cached `sector_pe_snapshot.json`/`industry_pe_snapshot.json` and flags `abs(drift_pct) > 0.25` as a data-quality signal (`comp_set_drift_flag` stderr log lines) — never a hard failure, no DB writes. Zero LLM. |
