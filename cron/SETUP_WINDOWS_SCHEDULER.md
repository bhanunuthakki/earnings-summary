...
Eighteen scheduled tasks total. The five daily ones run as a chain (03:00 → 06:30); a sixth daily task drains the LLM artifact queue at 04:00 and a seventh runs the Personal CIO morning pipeline at 04:00 (news → triggers → feed → validation); an eighth (02:45) backs up the database before the chain starts; the hourly catch-up is independent; the weekly + monthly tasks run off-cycle (the monthly `backup_es_monthly` writes a whole-project archive to the Drive-synced Downloads folder on the 1st at 01:00) and refresh the synthesis / lens layer, the IR-spreadsheet KPI series, and the IR-document corpus — including a twice-weekly rescan of the names whose IR crawl is still failing (a bot-protected site that may start cooperating); a quarterly task mines the rostered 13F managers one to two days after each 13F filing deadline; and a weekly Thursday audit verifies every task XML is registered and on schedule.
...
| `earnings-summary\backup_es_monthly` | Monthly, 1st @ 01:00 | `backup_es_monthly.task.xml` | `run_backup_es_monthly.bat` | **Whole-project offsite backup via the Drive-synced Downloads folder.** The `scratch` tree is intentionally NOT continuously Drive-synced (it corrupted git/worktrees), so this drops ONE archive per month into `Downloads\earnings-summary-backups\` where Drive uploads a single file. Runs `cron/backup_es_monthly.ps1`: (1) a consistent SQLite snapshot of `data/portfolio.db` (online backup API, via `cron/snapshot_db.py`) so a mid-write live DB is never torn; (2) a streamed **Zip64** archive of the whole project via .NET `ZipArchive` (NOT `Compress-Archive`, which is capped at 2 GB in PS 5.1) — default `scope=full` includes the re-fetchable FMP lake + `ir_documents` + `transcripts` (~8 GB zip); (3) a **hard credential-guard** that aborts the run (leaving last month's backup untouched) if any archive entry looks like a secret; (4) an atomic OVERWRITE of `earnings-summary-backup.zip` (single stable file; retention keeps the newest `ES_BACKUP_RETAIN`, default **1**, to avoid stacking ~8 GB copies). Excludes `venv/.git/.tmp/output/__pycache__/.claude/_inbox/node_modules/secrets/` + raw/`.bak` DBs + `*.pem/*.key/.env/credentials.json/token.json/gsheets_*`. Env overrides: `ES_BACKUP_SCOPE` (`full`\|`lean`), `ES_BACKUP_DEST`, `ES_BACKUP_RETAIN`, `ES_REPO_ROOT`. Runs an hour before the 02:45 daily DB backup so it never overlaps the 03:00 refresh-write window. |
...
schtasks /create /tn "earnings-summary\backup_es_monthly" ^
  /xml "%USERPROFILE%\.gemini\antigravity\scratch\earnings-summary\cron\backup_es_monthly.task.xml" ^
  /ru "%USERNAME%"
