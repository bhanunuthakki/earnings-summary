...
cd /d "%PROJECT_ROOT%"
call "%PROJECT_ROOT%\cron\run_python.bat" "refresh-portfolio-tracker" "portfolio-tracker-refresh" execution\refresh_portfolio_tracker.py --repo-root "%PROJECT_ROOT%" --api-url "%PORTFOLIO_TRACKER_API_URL%" --scheduled-task > "%LOG_FILE%" 2>&1
set "RC=%ERRORLEVEL%"
endlocal & exit /b %RC%
