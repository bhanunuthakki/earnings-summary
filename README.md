...
- **Comments / chat system** — start `start_comments_server.bat` (or `python execution/comments_server.py`) and open any `_workspace.html` brief. Free-text comments are persisted under `data/report_comments/<TICKER>/<YYYY-MM-DD>.json`. Slash-keyword shortcuts at the start of a comment route the intent: `/kpi`, `/thesis`, `/q`, `/ask`, `/fix`, `/update`, `/rewrite`, plus `/platform` / `/feature` / `/bug` for cross-workspace items (see below). `process_comments.bat` drains pending comment-driven edits via `execution/process_report_comments.py`.
- **Cross-workspace backlog** — comments classified as `platform_change` (either by the `/platform` / `/feature` / `/bug` keyword, by explicit phrases like *"across reports"* / *"across workspaces"* in the body, or by the Haiku classifier detecting a structural rather than name-specific symptom) are NOT routed into single-ticker brief edits. They land as area-tagged entries in [`directives/platform_backlog.md`](directives/platform_backlog.md) — the canonical tracker for cross-workspace bugs and feature requests. Group by area (`renderer` / `saydo` / `pipeline` / `kpi` / `chart` / `dcf` / `general`) when planning engineering work; every entry should ship as ONE renderer/pipeline change applied across all workspaces, not as a per-ticker brief edit. **When the user asks about open project work, check this file first** — it's the discoverable source-of-truth for outstanding cross-workspace features and bugs.
