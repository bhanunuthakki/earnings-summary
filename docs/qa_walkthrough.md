...
| `LEDGER_WORLDVIEW_ANCHOR` | **off** (`llm/anchors.py:543`) | Tenet-anchor injection into LLM prompts (Socratic, durable Ask context, workspace data, anchors) | prod has 0 Tenets → anchor inert even when on; verify no prompt bloat when empty |
...
| `PORTFOLIO_TRACKER_API_URL` / `PORTFOLIO_TRACKER_UI_URL` / `PORTFOLIO_TRACKER_ROOT` / `PORTFOLIO_TRACKER_SNAPSHOT_PATH` | explicit only; no baked sibling or port default | typed tracker reads, UI deep links, activation root, or immutable detached snapshots | absent/invalid source must render unavailable or stale; activation requires API_URL + ROOT |
...
## Services & processes
**Reach:** :7421 → `start_comments_server.bat` or the supervised `es-dashboard` service (Flask, loopback), both routed through `execution/sqlite_bootstrap.py`. Poller → supervised `es-poller` or the canonical generated scheduler wrapper, also bootstrap-managed. Scheduled fleet → `cron/*.task.xml` (UTF-16; edit via Python, and editing the XML ≠ the live registered task). **Preconditions:** Flask installed; `data/secrets/telegram_bot_token` for the poller; tasks registered in Windows Task Scheduler.
**Renders (top→bottom):** :7421 serves every request-time surface: `POST/GET/PATCH/DELETE /comments`, durable `POST /api/ask/stream` (SSE), `/api/ask/sessions*`, governed proposal review/decision routes, Work OS portfolio/company/brief reads, `/api/panel/<name>` operational fragments, `/api/peek/*`, `/api/tenets`, `/socratic/<T>`, `/review` doorways, `/actions/<name>` + `EventSource /actions/stream/<id>`, `/source/<doc_id>` viewer, `GET /healthz`. Legacy `/chat/<ticker>` routes are non-writing `410` handoffs only. The capture poller long-polls Telegram: ingests musings/voice (Whisper), runs the intent tap, artifact brief, wondering-detect, `/review <T>` command, coach-ping callbacks (`cp:review:`/`cp:dismiss:`), ledger jump-chips. **Both are long-lived Python processes: edits under `src/` require a restart to take effect (PR #815 lesson — poller especially).** Scheduled tasks that feed QA surfaces include `run_morning_pipeline`, `coach_pings`, `ledger_synthesis`, `weekly_p2_lens_refresh`, `monthly_calibration_scorecard`, `grade_calibration`, `backup_db`, `refresh_dirty_artifacts`, `daily_fetch_and_brief`, and `weekly_score_stances`. `verify_cron` audits registration; operational health is loaded on demand rather than polled by the home page.
...
| Start poller manually | `python execution/sqlite_bootstrap.py execution/capture_poller.py --once` | drains one Telegram batch, prints counts to stderr, exit 0 | token file absent → "not configured (…); exiting cleanly", exit 0 | P0 |
...
**Renders (top→bottom):** BUILD-time: the complete standalone research report plus the immutable shared body/snapshot artifact consumed by the Work OS reader. Request-time dependencies are comments, Work OS Copilot (`/api/ask/stream`), governed proposal decisions, Work OS portfolio/company/brief reads, operational panel fragments, peek popovers, action progress, socratic memo pages, review doorways, and source viewers.
...
**Reach:** `GET http://localhost:7421/` (`execution/comments_server.py` renders `src/pipeline/work_os_shell.py`). **Preconditions:** managed server running (`start_comments_server.bat`, or `es-dashboard`) and `data/portfolio.db` present for Cockpit rows.
...
**Reach:** Work OS Copilot (`src/pipeline/work_os_copilot.py`). **Preconditions:** `POST /api/ask/stream` (SSE frames session/stage/delta/fragment/final/citations/error); threads via `/api/ask/sessions` CRUD.
...
| Work OS hydration | page load | `GET /api/work-os/portfolio` populates the Cockpit once | Server down → explicit unavailable state; no retry storm | P1 |
...
| Tracker ↗ | click | new tab `${PORTFOLIO_TRACKER_UI_URL}/holdings?ticker=T` (matching row highlighted and scrolled into view) | link absent when explicit `PORTFOLIO_TRACKER_UI_URL` is not configured | P2 |
...
- Tracker unavailable/stale: position evidence says which configured API or snapshot source failed; it never becomes a not-held claim.
...
**Reach:** `#portfolio_risk`; fragment `GET /api/panel/portfolio_risk`. **Preconditions:** :7421 server. The explicitly configured tracker API feeds beta/drawdown/factor/risk-reward-gap; style/correlation/tail-stress/collision/macro-stress compute from local disk/DB and render typed tracker-unavailable evidence. Scenario run needs `execution/run_scenario.py` + LLM budget.
...
**Reach:** `#portfolio` (section landing); fragment `GET /api/panel/portfolio?start_date&end_date&include_backfill=1`. **Preconditions:** :7421; an explicitly configured tracker API/UI endpoint. Start is an ownership-checked action and does not imply health until a typed HealthV1 receipt proves it.

**Environment truth:** configure `PORTFOLIO_TRACKER_API_URL` (`API_URL`) for the tracker API, `PORTFOLIO_TRACKER_UI_URL` (`UI_URL`) for its web doorway, and `PORTFOLIO_TRACKER_ROOT` for the explicitly owned activation checkout. Portfolio snapshots must show source identity, snapshot date, and current/stale/unavailable state; an empty holdings list is not evidence that a ticker is not held when health and positions dates are one-sided or stale. The tracker start action is ownership-checked and does not itself prove listener or refresh health.

The legacy analytics facade retains its historical loopback compatibility
fallback for older read-only callers; QA of the typed runtime and activation
route must set `PORTFOLIO_TRACKER_API_URL` explicitly.

**Renders:** tracker-down → leading **Portfolio tracker** banner (warn-edged, "Start tracker" button, "starting automatically…" msg, `<details>` "Activation recovery · technical detail" with Scheduler-task recovery guidance and an explicit warning not to launch uvicorn directly). Tracker up → **Performance vs benchmarks** (header with hover "i" methodology popover + window bar: 1M/3M/6M/YTD/1Y/2Y/Default preset buttons, two date inputs, "Apply", "modeled backfill" checkbox w/ tooltip; KPI cards Portfolio TWR + vs SPY; color-keyed legend chips Portfolio/SPY/QQQ/Policy; multi-series SVG chart; policy-mix line, unbalanced warning; backfill-unreliable ⚠ note); **Risk & efficiency**; **Positioning & concentration** (Positions/Top1/Top5/Top10/HHI/Effective holdings/Avg corr cards + By asset type/sector/region/account bar blocks); **Per-position alpha** (living-grid filter + sortable table Ticker/Name/Value/P&L/SPY P&L/α vs SPY/α vs QQQ[/α vs policy] + totals row, ⚠ incomplete-row flags); **Live portfolio** (total + tax-bucket KPI strip, sortable positions table, **Latest transactions** table). Failed analytics endpoints listed once: "Unavailable from the tracker right now: …".
...
| "Start tracker" (also auto-fires once per page load) | click / auto | POST `/actions/start-tracker` → Scheduler activation plus a separate typed activation receipt; polls `/api/panel/portfolio` every 3s ×30 until `pf-live-offline` is gone, then reinjects (or `location.reload()`) | 409 ownership conflict; 400 unsupported API bind; 503 typed Scheduler failure or failed/unhealthy/stale/clock-incoherent HealthV1; no response is treated as success without fresh supervisor proof | P0 |
...
---
Key files: `src/pipeline/provenance_panel.py`, `src/pipeline/work_os_shell.py`, `dashboard_html.py`, per-section panels under `src/pipeline/`, and the operational routes in `execution/comments_server.py`.
...
| Affordance | Trigger | Expected result | Edge | Pri |
|---|---|---|---|---|
| Open | click report Chat / Open in Copilot / fact doorway | embedded report calls the parent `openWorkOsCopilot`; standalone report opens `/?copilot=1&ticker=<T>&report_date=<D>#screen-workspace`; company/report context arrives in the single durable workspace | server unavailable → clear Work OS link remains; iframed launcher avoids a second composer | P0 |
| Send | submit in Work OS Copilot | `POST /api/ask/stream` persists the exchange before terminal SSE; deltas, governed fragments, citations, and typed proposal references render on the owning assistant turn | typed error/replay state remains attached to the exchange | P0 |
| Fact doorway | click governed datum | opens Copilot with the exact `fact_ref`, company, report date, and a bounded research prompt | ref missing on host → no-op | P0 |
| Thesis/KPI proposal | Review exact change → Approve / Keep current | `GET /api/research/proposals/<id>` renders exact current→proposed values; explicit decision POST uses proposal revision + stable request id; approved state updates in place | revision conflict or target drift fails closed and disables stale actions | P0 |
...
**Reach:** same artifact both ways. **Preconditions:** managed server = `start_comments_server.bat` or `es-dashboard` (hardcoded `http://localhost:7421` boot URL for standalone files).
...
4. **Tracker-UP renders:** with the explicitly configured API running — sizing-audit live weights/alpha, Skill decomposition, Risk live vs cached-snapshot banner, tax lines in the review peek.
