...
**Reach:** Ctrl/Cmd+`.` from any tab (toggle). **Preconditions:** POSTs `/api/capture/text` (LLM-free ingest, channel `tray`); wondering-chip creation requires `LEDGER_RESEARCH_TAP` on (`tap_enabled()`); pledge challenge/receipt from `research.pledge` (failures swallowed — capture never breaks).
...
**Renders (top→bottom):** for each message the bot replies with exactly one confirm string from `_CONFIRM` (plain ASCII, no emoji): landed text/voice → `Captured.` or `Captured. ({TICKER})` when the roster matcher resolved one; ambiguous → `Captured. (Which ticker? Tap a candidate in the Ledger.)`; URL/document → `Saved to On My Mind.`; voice failure → `Couldn't transcribe that one - I kept the audio, try again?` / `That voice note came through empty - try again?`. Duplicate (`tg:<update_id>` dedup) or empty gets **silence**. When `LEDGER_ONMYMIND` feed flag is on, a landed capture's confirm carries the 4-button On My Mind ladder keyboard.
...
Three fix sessions were in flight when this doc was generated; check whether their PRs have merged before testing the affected rows.

1. **Coach P&L "decisions changed" counter integrity** (chip task_e319c6a4) — `_coach_changed_count` counts a guard_override memo as "changed" without requiring the 30-day window to have ELAPSED, owner inaction auto-satisfies it, and the only prod position_review memo (advisor_memos id 6, RBRK) is an agent verification artifact. Until fixed: expect the counter to be untrustworthy; do not treat "Decisions changed: N" as real.
2. **Coach real-path gaps** (chip task_e1136400) — (a) pledge regex misses "taking profits / exiting / lightening up / dumping / cutting"; (b) a falsifier_breach ping whose Telegram send FAILS parks in the digest lane forever (once-per-moment UNIQUE key, no re-send); (c) Telegram needs_ticker still deflects to the web Ledger (no in-channel candidate keyboard); (d) the System status dot never checks its artifact's age — a dead verify_daily_chain task leaves a stale green dot.
3. **Freshness/hedging honesty** (chip task_f586daee) — (a) verdict badge stale-grey compares against CALENDAR quarter-end, not print/ingestion date: expect fake-green in the 4-7 weeks after a print lands; (b) the graded-sells line asserts a hindsight-labeled n=6 record unhedged (bypasses calibration_guard's MIN_CONFIDENT_N=10 / rate_phrase).
4. **Position-tab guard literal** (chip task_d03a849f, fix session ran — verify merged) — "Guard: never run on this name" was an unconditional string beside the live review count; confirm it now derives from the count.
