...
- Automated proof: the implementation and repair commits were syntax-compiled and `git diff --check` passed. Ruff, strict Pyright, targeted pytest, and design sync require the project virtualenv; their receipts must be captured from `/Applications/earnings-summary/.venv` before release.
