...
- Automated proof through implementation anchor `9961a21b8bedea9c5ddab7c04639cc20ce5c88ea`: Ruff format/check passed; strict Pyright passed with zero errors; 229 targeted Company Desk, DCF-route, shell, Performance & Risk, policy, allocation, and mockup tests passed; design sync and `git diff --check` passed. Sol specialist and independent risk review both returned PASS under Judge policy 1.1.0.
- Browser proof: the prior isolated-build review covered desktop and 390px mobile hierarchy, stacking, no document overflow, Contracts/Say-Do roving-tab keyboard behavior, and zero console errors. It was not a live Windows production hydration or external-client compatibility proof.
- Remaining activation proof: verify the response contract and populated live-data hydration from the Windows production-shaped host, plus the Mac browser through the exact Tailscale Serve HTTPS origin. Confirm both the legacy external client (if retained) and the new Company Desk client consume the expanded response without relying on the removed Say/Do route. This does not authorize activation.
