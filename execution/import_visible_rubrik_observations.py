"""Validate an automatic visible-Chrome Rubrik export and write a sealed IR bundle."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        bundle = import_rubrik_visible_browser_export(source_bytes)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        _write_atomically(output_path, bundle.to_bytes())
    except Exception as exc:
        _event(
            "rubrik_visible_browser_export_import_failed",
    ...
    _event("rubrik_visible_browser_export_import_completed", output=str(output_path))
    return 0
...
def _write_atomically(path: Path, content: bytes) -> None:
    descriptor, temporary_name = tempfile.mkstemp(prefix=".rubrik-visible-import-", dir=path.parent)
