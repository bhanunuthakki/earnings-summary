...
def sweep_db(conn: sqlite3.Connection, *, apply: bool) -> Counter[str]:
    ...
    for table, id_col, col in _DB_TARGETS:
        target = f"{table}.{col}"
        rows = conn.execute(f"SELECT {id_col} AS id, {col} AS val FROM {table}").fetchall()  # nosec B608 -- table/column names come from the _DB_TARGETS literal; values stay bound
        for row in rows:
            ...
            if apply:
                conn.execute(f"UPDATE {table} SET {col} = ? WHERE {id_col} = ?", (plain, row["id"]))  # nosec B608 -- table/column names come from the _DB_TARGETS literal; values stay bound
