...
def main() -> int:
    ...
    try:
        agg = run_quarterly_segment_dcf_for_ticker(
            conn,
            ticker=args.ticker,
        ...
        "components": [
            {
                ...
                "ttm_base_revenue_billions": c.inputs.base_revenue / 1e9,
                "fcf_margin": c.inputs.fcf_margin,
                "first_year_growth_avg": (
                    sum(c.inputs.revenue_growths[:4]) / 4
                    if len(c.inputs.revenue_growths) >= 4
                    else None
                ),
