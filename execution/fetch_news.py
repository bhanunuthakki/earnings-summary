...
Drives the two feeds into the `news` table under one policy:
...
Both feeds map into one validated NewsRow and write through a single
``upsert_news_rows`` call, so ``(ticker, url)`` dedup means a story seen by both
feeds is stored once.
...
Usage:
    python execution/fetch_news.py                       # auto, active book
    python execution/fetch_news.py --source websearch
...
from news.store import NewsRow, upsert_news_rows  # noqa: E402
...
def run(tickers: list[str], *, source: str, db_path: str, days: int, limit: int) -> int:
    """Collect every ticker under the source policy and persist through one
    connection. Returns 0 normally; 1 only on a structural failure (the `news`
    table absent) — per-ticker feed hiccups are logged and degraded, so the
    morning pipeline's trigger stage still runs."""
    if not tickers:
...
def main(argv: list[str] | None = None) -> int:
    ...
    return run(
        tickers, source=cast("str", args.source), db_path=db_path, days=args.days, limit=args.limit
    )
