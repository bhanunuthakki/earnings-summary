...
def build(s: Assum, m: Mirror, dest: Path) -> None:
    ...
    scenarios = [
        ("Bear", 0.15, 0.16, 0.13),
        ("Base", s.comm_g_near, s.comm_margin_term, s.nimal_term),
        ("Bull", 0.28, 0.24, 0.17),
    ]
    r = 3
    for name, cg, cm, nm in scenarios:
        s2 = copy.copy(s)
        s2.comm_g_near, s2.comm_margin_term, s2.nimal_term = cg, cm, nm
        v = mirror(s2).vps
        scn.cell(row=r, column=1, value=name).font = Font(bold=(name == "Base"), color="374151")
        for col, vv in zip(("B", "C", "D"), (cg, cm, nm), strict=True):
            cc = scn[f"{col}{r}"]
            cc.value = vv
