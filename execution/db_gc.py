...
def _archive_doomed(
    ...
    *,
    ...
    """
    conn.execute(
        f'CREATE TABLE IF NOT EXISTS gcarc."{table}" AS SELECT * FROM main."{table}" WHERE 0'  # nosec B608 -- closed internal table set; no external input
    )
    cur = conn.execute(
        f'INSERT INTO gcarc."{table}" SELECT t.* FROM main."{table}" t '  # nosec B608 -- closed internal table set; values remain bound
...
def _reset_doomed(conn: sqlite3.Connection, name: str = "_gc_doomed") -> None:
    ...
    conn.execute(f'DELETE FROM "{name}"')  # nosec B608 -- fixed temp-table name; no external input
...
def telemetry_retention(
    ...
    for table, ts_col in TELEMETRY_TABLES:
        ...
        _reset_doomed(conn)
        conn.execute(
            f'INSERT INTO _gc_doomed (id) SELECT "{id_col}" FROM "{table}" WHERE "{ts_col}" < ?',  # nosec B608 -- TELEMETRY_TABLES is a closed set; values remain bound
            (cutoff,),
        )
        ...
        if apply and n:
            ...
            conn.execute(f'DELETE FROM "{table}" WHERE "{id_col}" IN (SELECT id FROM _gc_doomed)')  # nosec B608 -- TELEMETRY_TABLES is a closed set
...
def _doom_facts_for_ticker(
    ...
    *,
    ...
    keep_periods = [
        r[0]
        for r in conn.execute(
            f"SELECT DISTINCT period_end FROM financial_facts "  # nosec B608 -- qmarks is internally-built placeholders; values remain bound
    ...
    keep_years = [
        r[0]
        for r in conn.execute(
            f"SELECT DISTINCT substr(period_end, 1, 4) FROM financial_facts "  # nosec B608 -- amarks is internally-built placeholders; values remain bound
    ...
    cur = conn.execute(
        ...
        """,  # nosec B608 -- placeholder lists built internally; values remain bound
...
def facts_depth(
    ...
    *,
    ...
    for tbl, table_col, id_col in (
        ...
        try:
            cur = conn.execute(
                f'DELETE FROM "{tbl}" WHERE "{table_col}" = ? '  # nosec B608 -- closed internal table/column set; values remain bound
