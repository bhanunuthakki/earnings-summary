...
# --- context ---
lines = [f"Company: {prof.get('companyName', T)} ({T})",
...
for y in full[-3:]:
    ...
    lines.append(f"  FY{y}: revenue {rev:,.0f}  op-margin {oi / rev:.1%}  net-margin {ni / rev:.1%}  "
...
if fwd:
    ...
    for y in fwd:
        e = ebY[y]
        lines.append(f"  {y}: revenue {m(e.get('revenueAvg')):,.0f}  net-income {m(e.get('netIncomeAvg')):,.0f}  EPS {e.get('epsAvg', '?')}")
...
print(f"OK\t{T}\tapplicable={data.get('dcf_applicable')}\tmodel={data.get('business_model')}\t"
