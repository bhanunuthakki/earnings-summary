"""Sample: a Damodaran/MBI-style, FORMULA-DRIVEN DCF workbook for AMZN (v3).

Sheets: Cover, Color Code, WACC, Model, Financials, Consensus, Valuation.
Everything derived is an Excel/Sheets formula. Literals only for blue=actuals
and yellow=assumptions; green=cross-sheet link. Default terminal = exit multiple,
with perpetuity kept as a cross-check. Terminal method + exit basis are dropdowns.
Produces dcf/_sample_AMZN_redesign.xlsx.
"""
...
REPO = Path(r"C:\Users\Bhanu\.gemini\antigravity\scratch\earnings-summary")
FMP = REPO / "data" / "historical" / "fmp"
import os  # noqa: E402

T = os.environ.get("DCF_TICKER", "AMZN")
_pfd = json.loads((FMP / f"{T}_profile.json").read_text(encoding="utf-8")) if (FMP / f"{T}_profile.json").exists() else [{}]
_pfd = (_pfd[0] if isinstance(_pfd, list) and _pfd else _pfd) if isinstance(_pfd, (list, dict)) else {}
NAME = os.environ.get("DCF_NAME") or (_pfd.get("companyName") if isinstance(_pfd, dict) else None) or T
DEST = Path(os.environ.get("DCF_DEST", str(REPO / "dcf" / "_sample_AMZN_redesign.xlsx")))
...
PROD = sorted((s for s, v in _latest_p.items() if isinstance(v, (int, float))), key=lambda s: -_latest_p[s])
GEO = sorted((s for s, v in _latest_g.items() if isinstance(v, (int, float))), key=lambda s: -_latest_g[s])
...
other_ly = (rev_ly - fy_sum_raw(inc_i, "costOfRevenue", ly) - fy_sum_raw(inc_i, "researchAndDevelopmentExpenses", ly)
            - fy_sum_raw(inc_i, "sellingGeneralAndAdministrativeExpenses", ly) - oi_ly) / rev_ly
capex_da_ly = abs(fy_sum_raw(cf_i, "capitalExpenditure", ly)) / fy_sum_raw(cf_i, "depreciationAndAmortization", ly)
...
for e in est:
    try:
        ...
        pass
cons_rev = {y: m(est_by_year[y].get("revenueAvg")) for y in FC_YEARS if (est_by_year.get(y) or {}).get("revenueAvg")}
cons_ni = {y: m(est_by_year[y].get("netIncomeAvg")) for y in FC_YEARS if (est_by_year.get(y) or {}).get("netIncomeAvg")}
...
for j, y in enumerate(FC_YEARS):
    if y in cons_rev:
        ...
        delta = (cons_rev[y] - sum(seg_prev[s] * (1 + mg[s]) for s in PROD)) / denom if denom else 0.0
        for s in PROD:
            gg = mg[s] + delta
            seg_growth[s][j] = gg
            seg_prev[s] *= 1 + gg
...
for j, y in enumerate(FC_YEARS):
    ...
    other_pct[j] = max(1 - ratios_ly["cogs"] - ratios_ly["rnd"] - ratios_ly["sga"] - target_oim, 0.02)
...
FX = {"USD": 1.0, "DKK": 0.145, "EUR": 1.08, "GBP": 1.27, "CAD": 0.73, "BRL": 0.18,
      "ILS": 0.27, "INR": 0.012, "JPY": 0.0067, "CHF": 1.12, "SEK": 0.095}.get(CURRENCY, 1.0)
_opus = (json.loads(cache.read_text(encoding="utf-8")).get("redesign") if cache.exists() else None) or {}
...
if _opus.get("segments"):
    ...
    if _opus.get("capex_pct_revenue_2026"):
        CAPEX_2026_M = _opus["capex_pct_revenue_2026"] * (cons_rev.get(FC_YEARS[0], rev_ly) or rev_ly)
    cda0 = CAPEX_2026_M / _da_2026
...
_tmetric = {"EV/EBITDA": oi_p[-1] + da_p[-1], "EV/Sales": rev_p[-1], "EV/EBIT": oi_p[-1],
            "EV/FCF": vf_p[-1]}.get(OPUS_BASIS, oi_p[-1] + da_p[-1])
...
full_value = (sum(vf_p[t] / (1 + wacc0) ** (t + 1) for t in range(N_FC))
              + _tv / (1 + wacc0) ** N_FC + cash_now - debt_now) / shares_now * FX

ann_rev = [fy_sum_raw(inc_i, "revenue", y) for y in full_fys]
ann_g = [ann_rev[i] / ann_rev[i - 1] - 1 for i in range(1, len(ann_rev))]
ann_oim = [fy_sum_raw(inc_i, "operatingIncome", y) / fy_sum_raw(inc_i, "revenue", y) for y in full_fys]
cagr0 = (rev_p[-1] / rev_ly) ** (1 / N_FC) - 1
mT0 = oi_p[-1] / rev_p[-1]
SIG = {"cagr": float(min(max(np.std(ann_g) * 0.5, 0.012), 0.03)), "margin": float(min(max(np.std(ann_oim), 0.015), 0.03)),
       "wacc": 0.005, "exit": 1.5}
...
def _reduced(cagrs, mTs, waccs, exits):
    ...
    return (pv + (ebit[:, -1] + d[:, -1]) * exits / (1 + waccs) ** N_FC + cash_now - debt_now) / shares_now


base_red = float(_reduced(np.array([cagr0]), np.array([mT0]), np.array([wacc0]), np.array([EXIT_MULT]))[0])
kcal = full_value / base_red if base_red else 1.0
rng = np.random.default_rng(42)
NMC = 10000
mc_vals = _reduced(
    rng.normal(cagr0, SIG["cagr"], NMC), rng.normal(mT0, SIG["margin"], NMC),
    np.clip(rng.normal(wacc0, SIG["wacc"], NMC), 0.05, 0.16), np.clip(rng.normal(EXIT_MULT, SIG["exit"], NMC), 5, 24),
) * kcal
PCTS = [5, 10, 25, 50, 75, 90, 95]
mc_res = {"mean": float(mc_vals.mean()), "median": float(np.median(mc_vals)), "std": float(mc_vals.std()),
          "min": float(mc_vals.min()), "max": float(mc_vals.max()),
          "pcts": {p: float(np.percentile(mc_vals, p)) for p in PCTS},
          "p_under": float((mc_vals > price).mean()), "p_up20": float((mc_vals > price * 1.2).mean())}
...
DB = {
    ...
    "margin_near": "Dashboard!$B$29", "margin_term": "Dashboard!$B$30", "tax": "Dashboard!$B$31",
    "capex26": "Dashboard!$B$34", "capex_term_da": "Dashboard!$B$35",
    "rf": "Dashboard!$B$38", "erp": "Dashboard!$B$39", "beta": "Dashboard!$B$40", "kd": "Dashboard!$B$41",
    "method": "Dashboard!$B$43", "basis": "Dashboard!$B$44", "mult": "Dashboard!$B$45", "tg": "Dashboard!$B$46",
...
write_yoy(rev_r)
for lab, fld in [("Cost of Revenue", "costOfRevenue"), ("R&D Expense", "researchAndDevelopmentExpenses"),
...
fin_row["Diluted Shares (M)"] = write_qrow("Diluted Shares (M)", lambda i, k: m(inc_i.get(k, {}).get("weightedAverageShsOutDil")))
...
frow += 1
for lab, fld in [("Cash & ST Investments", "cashAndShortTermInvestments"), ("Total Current Assets", "totalCurrentAssets"),
                 ("PP&E (net)", "propertyPlantEquipmentNet"), ("Total Assets", "totalAssets"),
...
frow += 1
for lab, fld in [("D&A", "depreciationAndAmortization"), ("Stock-based Compensation", "stockBasedCompensation"),
                 ("Change in Working Capital", "changeInWorkingCapital"), ("Operating Cash Flow", "operatingCashFlow"),
                 ("Capex", "capitalExpenditure"), ("Free Cash Flow", "freeCashFlow")]:
    fin_row[lab] = write_qrow(lab, lambda i, k, f=fld: m(cf_i.get(k, {}).get(f)))
put(fs, frow + 1, 1, "Blue = hardcoded actuals from FMP filings. Same-sheet ratios are formulas.").font = SUB
...
wrows = [
    ...
    ("Market cap (E)", f"={DB['price']}*Financials!{LAST}{fin_row['Diluted Shares (M)']}", USD, "f"),
...
put(wc, WACC_ROW + 2, 1, "Yellow = edit. CAPM cost of equity; market-value weights. Feeds Valuation!WACC.").font = SUB
...
r = 3
band(md, r, "REVENUE BY SEGMENT (yellow = growth assumption; actuals link to Financials)", len(ALL) + 1)
r += 1
seg_rev_row = {}
for i, s in enumerate(PROD):
    ...
    for y in FC_YEARS:  # hold near-term ~3y then fade to terminal (green link to Dashboard)
        put(md, r, yc[y], f"={DB['g1'](i)}+({DB['gT'](i)}-{DB['g1'](i)})*MAX(0,{fcj[y]}-2)/{N_FC - 3}", fmt=PCT)
    r += 1
rev_row = r
...
r += 1
cost_rows = {}
for lab, fin_lab, ser in [("Cost of revenue", "Cost of Revenue", cogs_pct), ("R&D / Technology", "R&D Expense", rnd_pct),
...
for y in ALL:
    put(md, r, yc[y], f"={mc(y)}{nopat_row}+{mc(y)}{da_row}+{mc(y)}{sbc_row}-{mc(y)}{nwc_row}-{mc(y)}{capex_row}", fmt=USD, bold=True)
r += 1
valfcf_row = r
...
band(md, r, "RETURNS ON CAPITAL & EFFICIENCY  (is new revenue/capital earning its keep?)", len(ALL) + 1)
r += 1
EQ_F, DB_F, CA_F = fin_row["Total Equity"], fin_row["Long-term Debt"], fin_row["Cash & ST Investments"]
...
for y in full_fys:
    put(md, r, yc[y], f"=Financials!{q4(y)}{EQ_F}+Financials!{q4(y)}{DB_F}-Financials!{q4(y)}{CA_F}", fmt=USD)
for y in FC_YEARS:
    put(md, r, yc[y], f"={mc(y - 1)}{ic_row}+{mc(y)}{capex_row}-{mc(y)}{da_row}+{mc(y)}{nwc_row}", fmt=USD)
r += 1
eq_row = r
...
for y in ALL:
    if (y - 1) in yc:
        put(md, r, yc[y], ie(f"({mc(y)}{oi_row}-{mc(y - 1)}{oi_row})/({mc(y)}{rev_row}-{mc(y - 1)}{rev_row})"), fmt=PCT)
r += 1
put(md, r, 1, "  ROIIC (ΔNOPAT / ΔInvested capital)", bold=True)
for y in ALL:
    if (y - 1) in yc:
        put(md, r, yc[y], ie(f"({mc(y)}{nopat_row}-{mc(y - 1)}{nopat_row})/({mc(y)}{ic_row}-{mc(y - 1)}{ic_row})"), fmt=PCT)
r += 1
put(md, r, 1, "  Sales-to-capital (ΔRev / ΔInvested capital)")
for y in ALL:
    if (y - 1) in yc:
        put(md, r, yc[y], ie(f"({mc(y)}{rev_row}-{mc(y - 1)}{rev_row})/({mc(y)}{ic_row}-{mc(y - 1)}{ic_row})"), fmt='0.00"x"')
r += 1
...
for lab, fld, mexpr, fmt in metrics:
    ...
    r += 2
put(cs, r, 1, "Defaults anchored to consensus revenue + net income (≈0% diff). Other rows show where the model sits vs Street.").font = SUB
put(cs, r + 1, 1, "*FMP's EBITDA/EBIT estimates look unreliable for AMZN (it reports NI > EBIT) — don't over-read those two.").font = SUB
...
B = {"price": "$B$2", "wacc": "$B$3", "g": "$B$4", "tax": "$B$5", "method": "$B$6", "basis": "$B$7", "mult": "$B$8"}
...
put(vs, vps_row, 2, f"=B{br + 4}/B{br + 5}*{FX}", fmt=PXS).font = BIG  # *FX -> USD (1.0 for USD names)
...
put(mcs, 2, 1, f"{NTRIAL:,} trials, recomputed in-sheet on every edit. Edit the yellow driver distributions to re-roll. Engine grid is to the right (cols H+).").font = SUB
...
for i, (lab, mu, sd, fmt) in enumerate([("Revenue 10y CAGR", _cagr_link, SIG["cagr"], PCT),
...
for i, (lab, f) in enumerate([("Mean", f"=AVERAGE({VR})"), ("Median (P50)", f"=MEDIAN({VR})"),
                              ("Std dev", f"=STDEV({VR})"), ("Min", f"=MIN({VR})"), ("Max", f"=MAX({VR})"),
                              ("Current price", f"={PR}"), ("Base case (full model)", f"=Valuation!B{vps_row}")]):
    put(mcs, 12 + i, 1, lab)
...
chart.add_data(Reference(mcs, min_col=2, min_row=hb + 2, max_row=hb + 1 + NB), titles_from_data=False)
...
for i, (lab, v) in enumerate([
    ...
    ("da (D&A / revenue)", ratios_ly["da"]), ("cda0 (2026 capex/D&A)", cda0),
    ("nwc (% incr revenue)", 0.005), ("tax", "=Valuation!$B$5"), ("k (calibration)", kcal),
...
def val_formula(rs):
    return ("=$I$8*(SUMPRODUCT(($I$2*(1+K" + rs + ")^" + HARR + ")*(($I$3+(L" + rs + "-$I$3)*" + HARR
            + "/10)*(1-$I$7)+$I$4-$I$4*($I$5+(1.05-$I$5)*" + HARR + "/10)-(K" + rs + "/(1+K" + rs
            + "))*$I$6)/(1+M" + rs + ")^" + HARR + ")+$I$2*(1+K" + rs + ")^10*(L" + rs + "+$I$4)*N" + rs
            + "/(1+M" + rs + ")^10+$I$9-$I$10)/$I$11")
...
put(dsh, 2, 1, "Edit the yellow cells; everything recomputes. Orange = a driver you've moved off consensus.").font = SUB
...
guards = [
    ...
    ("Implied g vs exit multiple", f"={ig}", PCT, f'=IF({ig}>{DB["tg"]}+0.02,"! rich","ok")', "near term g"),
    ("ROIC - WACC (2026)", f"={roic26}-WACC!B{WACC_ROW}", PCT,
     f'=IF({roic26}-WACC!B{WACC_ROW}<0,"! destroys","creates")', "> 0"),
    ("Revenue vs consensus (2026)", "=Consensus!B6", PCT, '=IF(ABS(Consensus!B6)>0.05,"! off-Street","on")', "+/- 5%"),
...
seg_notes = {"Amazon Web Services": "AI/cloud reaccel + capacity adds", "Advertising Services": "ad load + retail media",
             "Online Stores": "share gains, GMV growth", "Third-Party Seller Services": "3P mix + fee/ad take",
             "Subscription Services": "Prime price + member adds", "Physical Stores": "grocery/format expansion",
...
put(dsh, 34, 5, "Amazon guided ~$200B (AI/datacenters)" if T == "AMZN" else "~last-year capex +10%; set to guidance").font = SUB
...
dsh.conditional_formatting.add(f"B{SEG_ROW0}:B{SEG_ROW0 + len(PROD) - 1}",
...
put(cc, 7, 2, "Edit yellow cells (Model assumptions, WACC drivers, Valuation toggles). Everything else recomputes.")
...
for i, (k, v) in enumerate([("Last updated", date.today().isoformat()), ("Data pulled (FMP)", qlabels[-1]),
...
for i, (k, v, fmt) in enumerate([("WACC (from WACC tab)", f"=WACC!B{WACC_ROW}", PCT), ("Terminal method", "=Valuation!$B$6", None),
                                 ("Exit basis / multiple", "=Valuation!$B$7", None), ("Exit multiple", "=Valuation!$B$8", MULT),
                                 ("Terminal growth (g)", "=Valuation!$B$4", PCT), ("Terminal weight", f"=Valuation!B{ck + 1}", PCT),
...
order = ["Cover", "Dashboard", "Color Code", "WACC", "Model", "Financials", "Consensus", "Valuation", "Monte Carlo"]
