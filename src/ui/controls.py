...
/* pressed/in-flight state — CCAction.busy() sets aria-busy. Slightly less dim
   than [disabled] so "working" reads apart from "unavailable". */
.k-btn[aria-busy="true"] { opacity: 0.6; pointer-events: none; }
...
.k-card-head { display: flex; align-items: flex-start; justify-content: space-between;
  gap: var(--sp-3); min-width: 0; }
.k-card-heading { display: flex; flex-direction: column; gap: var(--sp-1); min-width: 0; }
.k-card-section { display: flex; flex-direction: column; gap: var(--sp-3); }
.k-card-stat { padding: 0; }
.k-card-action { padding: var(--sp-2) var(--sp-3); }
.k-card-nav { display: flex; flex-direction: column; gap: var(--sp-1); }
...
.k-stat-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); }
.k-stat-cell { min-width: 0; padding: var(--sp-3); border-inline-start: var(--bw-thin) solid var(--hairline); }
.k-stat-cell:first-child { border-inline-start: 0; }
.k-section-head { display: flex; align-items: baseline; justify-content: space-between;
  gap: var(--sp-3); min-width: 0; }
.k-section-title { margin: 0; font-size: var(--fs-title); font-weight: 600; color: var(--fg); }
.k-section-meta { margin: 0; font-size: var(--fs-caption); color: var(--muted); }
.k-table-shell { min-width: 0; overflow: hidden; background: var(--surface);
  border: var(--bw-thin) solid var(--border); border-radius: var(--radius-card);
  box-shadow: var(--shadow-card); }
@media (max-width: 47.5rem) {
  .k-stat-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .k-stat-cell:nth-child(odd) { border-inline-start: 0; }
  .k-stat-cell:nth-child(n + 3) { border-block-start: var(--bw-thin) solid var(--hairline); }
  .k-card-head, .k-section-head { align-items: stretch; flex-direction: column; }
}
...
/* ---- Generation 3 Work OS Primitives ---- */
.k-desk-hero { display: flex; align-items: center; justify-content: space-between; gap: var(--sp-3); flex-wrap: wrap; padding: var(--sp-3); background: var(--surface); border: var(--bw-thin) solid var(--border); border-radius: var(--radius-card); box-shadow: var(--shadow-card); }
.k-say-do-timeline { display: flex; flex-direction: column; gap: var(--sp-2); padding: var(--sp-3); background: var(--surface); border: var(--bw-thin) solid var(--border); border-radius: var(--radius-card); }
.k-say-do-item { display: grid; grid-template-columns: auto 1fr auto; align-items: baseline; gap: var(--sp-3); padding: var(--sp-2) 0; border-bottom: var(--bw-thin) solid var(--hairline); }
.k-say-do-item:last-child { border-bottom: none; }
.k-matrix-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(var(--grid-card-md), 1fr)); gap: var(--sp-3); }
.k-action-card { display: flex; align-items: center; justify-content: space-between; gap: var(--sp-3); padding: var(--sp-3); background: var(--surface); border: var(--bw-thin) solid var(--border); border-radius: var(--radius-card); box-shadow: var(--shadow-card); }
.desk-stats-strip { display: flex; align-items: center; gap: var(--sp-4); flex-wrap: wrap; }
.desk-tabs-bar { display: flex; gap: var(--sp-2); border-bottom: var(--bw-thin) solid var(--border); padding-bottom: var(--sp-1); margin: var(--sp-2) 0; }
"""
