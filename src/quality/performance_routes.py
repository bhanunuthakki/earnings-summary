"""Deterministic Flask route workload and fixture instrumentation for BHA-115.

This module owns the route corpus, disposable fixture construction, and
causal measurements used by :mod:`execution.benchmark_performance_workload`.
Keeping those details behind one execution-facing function leaves the CLI
focused on selecting a workload and assembling its envelope.
"""
...
def _migrate(root: Path, database: Path) -> None:
    """Migrate a route fixture database through the real Alembic head."""
...
def _seed_route_fixture(root: Path, database: Path, fixture_repo: Path) -> dict[str, int]:
    ...
    put_session_context(ask_session.id, SessionContextV1(company_ticker="NU"), db_path=database)
    begin_exchange(
            ...
            with (
                ...
                for phase in ("cold", "warm"):
                    for route_index, route_name in enumerate(ROUTE_NAMES):
                        database = temp_root / f"{phase}-{route_index}.db"
                        ...
                        route_repo = temp_root / f"{phase}-{route_index}-repo"
                        ...
                        companions.append(
                            RouteCausalCompanion(
                                ...
                                external_call_hold_seconds=external_hold_seconds
                                - route_external_hold_seconds,
...
# Public implementation names keep the execution shim type-safe; its private
# aliases remain solely for compatibility with existing focused tests.
route_request = _route_request
routes = _routes
seed_route_fixture = _seed_route_fixture
