"""Measure the repository architecture contract deterministically.
...
baseline receipts reproducible even after the tree changes.  Score admission
is deliberately owned by a separate, trusted-evidence layer.
"""
...
class ArchitectureReceipt(StrictModel):
    schema_version: Literal["architecture-measurement-v1"] = SCHEMA_VERSION
    scoped_revision: str
    scoped_commit: str
    scanner_sha256: str
    source_sha256: str
...
def tracked_python_paths(repo_root: Path) -> tuple[str, ...]:
    result = subprocess.run(
        ["git", "ls-files", "-z", "--", "src", "execution"],
        cwd=repo_root,
        check=False,
        capture_output=True,
    )
    ...
    try:
        ...
        raise ValueError("git ls-files returned a non-UTF-8 path") from exc
    paths = {path for path in candidates if path.endswith(".py") and path.startswith(SOURCE_ROOTS)}
...
def build_architecture_receipt(repo_root: Path, revision: str) -> ArchitectureReceipt:
    ...
    return ArchitectureReceipt(
        ...
        definitions={
            ...
            "noncomment_loc": "nonblank physical line that is not a full-line COMMENT token",
...
def compare_architecture(
    current: ArchitectureReceipt, baseline: ArchitectureReceipt
) -> ArchitectureRatchetReceipt:
    """Compare two measurements without treating scanner drift as evidence."""
    regressions = architecture_regressions(current.metrics, baseline.metrics)
    scanner_mismatch = current.scanner_sha256 != baseline.scanner_sha256
    status: Literal["PASS", "FAIL", "HOLD"] = (
        "HOLD" if scanner_mismatch else ("FAIL" if regressions else "PASS")
    )
    return ArchitectureRatchetReceipt(
        schema_version="architecture-ratchet-v1",
        status=status,
        scoped_commit=current.scoped_commit,
...
def main() -> int:
    ...
    try:
        ...
        if baseline is None:
            ...
            return 0
        ratchet = compare_architecture(architecture, baseline)
