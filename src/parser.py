...
def parse_filename(filename):
    ...
    return {"company": company, "quarter": quarter, "year": year}
...
def smart_rename_files(input_dir):
    ...
    for filename in os.listdir(input_dir):
        ...
        heuristic_match = re.search(
            r"([a-zA-Z0-9\.]+)[ _-](Q[1-4])[ _-](20[2-3]\d)", filename, re.IGNORECASE
        )
        ...
        if heuristic_match:
            try:
                ...
                if len(h_company) > 1:
                    ...
                    print(
                        f"    [Heuristic] Identified {h_company}, {h_quarter}, {h_year} from filename."
                    )
