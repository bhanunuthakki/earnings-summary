...
The lane model (design_language "Diet-vs-alert"):
  ...
  * The DIET-ONLY types never alert — written DIRECTLY, never mirrored from
    ...
    ``investor_day`` (forward-dated, carries an ``event_date``) and the two
...
# Historical schema compatibility only. No active reader, writer, budget, or
# eval owns this retired podcast type. Existing rows remain retained evidence.
...
RETIRED_SIGNAL_TYPES: frozenset[str] = frozenset({SIGNAL_MEDIA_APPEARANCE})
ACTIVE_SIGNAL_TYPES: frozenset[str] = SIGNAL_TYPES - RETIRED_SIGNAL_TYPES
...
DIET_ONLY_TYPES: frozenset[str] = ACTIVE_SIGNAL_TYPES - NEWS_MIRRORED_TYPES
