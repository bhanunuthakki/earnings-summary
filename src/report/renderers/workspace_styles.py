...
(the smallest scale step since the four-step collapse); box corners ride the
one ``--radius`` (pills ``--radius-full``);
...
``--fs-caption`` floor and the chip shape: the 8.5px / 3px-corner per-number
...
.logo-sm { color: var(--muted); margin: 0 2px; }
.crumb {
...
.failure { display: flex; gap: 14px; padding: var(--row-pad-y) var(--panel-pad-x); border-bottom: 1px solid var(--hairline); }
...
.ledger-stale {
  margin-left: 6px; font-size: var(--fs-caption); font-weight: 600; text-transform: uppercase;
...
/* ============================================================
   ...
   ============================================================ */
/* Mirrors .l1-thesis's card treatment (same inset, same density) with a
   distinct accent so the two ledes read as separate registers — the
   thesis is the standing read, this is the latest delta. */
.l1-reread {
...
.l1-reread .reread-label {
  font-size: var(--fs-caption); text-transform: uppercase; letter-spacing: 0.08em;
  color: var(--warn); font-weight: 600; flex-shrink: 0;
}
.l1-reread p {
  margin: 0; flex: 1; font-size: 13.5px; line-height: 1.55; color: var(--fg-soft);
}
...
.l1-decision-card .dc-cell h4 {
  margin: 0 0 4px; font-size: var(--fs-caption); text-transform: uppercase;
...
.l1-decision-card .dc-blockers ul { margin: 4px 0 0; padding-left: 18px; }
...
.l1-decision-card .dc-status {
  font-size: var(--fs-caption); color: var(--fg-soft); margin-top: 4px;
}
...
/* No local margin: .lens-panel is a direct child of .tab-body, whose own
   ...
   STACK on top of that gap (24px + 16px = 40px between two lens panels)
...
.ws-kbd-hint { margin-top: 12px; font-size: var(--fs-caption); color: var(--muted); }
"""
)
